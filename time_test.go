...
func TestTime_MarshalJSON(t *testing.T) {
	tests := []struct {
		...
		{name: "present", params: Time{Value: time.Date(2023, 10, 5, 14, 48, 0, 0, time.UTC), Present: true}, want: `"2023-10-05T14:48:00Z"`},
		{name: "not present", params: Time{}, want: "null"},
		{name: "zero time but present", params: Time{Value: time.Time{}, Present: true}, want: `"0001-01-01T00:00:00Z"`},
		{name: "with milliseconds", params: Time{Value: time.Date(2023, 10, 5, 14, 48, 0, 123000000, time.UTC), Present: true}, want: `"2023-10-05T14:48:00.123Z"`},
		{name: "with nanoseconds", params: Time{Value: time.Date(2023, 10, 5, 14, 48, 0, 123456789, time.UTC), Present: true}, want: `"2023-10-05T14:48:00.123456789Z"`},
		{name: "with timezone offset", params: Time{Value: time.Date(2023, 10, 5, 14, 48, 0, 0, time.FixedZone("UTC+2", 2*60*60)), Present: true}, want: `"2023-10-05T14:48:00+02:00"`},
		{name: "not present with non-zero time", params: Time{Value: time.Date(2023, 10, 5, 14, 48, 0, 0, time.UTC), Present: false}, want: "null"},
		{name: "zero time and not present", params: Time{Value: time.Time{}, Present: false}, want: "null"},
		{name: "far future date", params: Time{Value: time.Date(3000, 1, 1, 0, 0, 0, 0, time.UTC), Present: true}, want: `"3000-01-01T00:00:00Z"`},
		{name: "far past date", params: Time{Value: time.Date(1000, 1, 1, 0, 0, 0, 0, time.UTC), Present: true}, want: `"1000-01-01T00:00:00Z"`},
		{name: "leap year date", params: Time{Value: time.Date(2020, 2, 29, 12, 0, 0, 0, time.UTC), Present: true}, want: `"2020-02-29T12:00:00Z"`},
		{name: "end of year", params: Time{Value: time.Date(2023, 12, 31, 23, 59, 59, 999999999, time.UTC), Present: true}, want: `"2023-12-31T23:59:59.999999999Z"`},
		{name: "start of year", params: Time{Value: time.Date(2023, 1, 1, 0, 0, 0, 0, time.UTC), Present: true}, want: `"2023-01-01T00:00:00Z"`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dst := tt.params
